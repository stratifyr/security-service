...
2. **Plan — Research and verify every claim:** Explore the referenced files; confirm the issue's statements are accurate (dead code really is unreferenced, structs really are triplicated, etc.). Use `go build ./... && go vet ./... && golangci-lint run ./...` at the start as a clean baseline.
...
7. **Build — Verify:** Run `go build ./... && go vet ./... && golangci-lint run ./...`.
...
12. **PR — Final verification:** Run `go build ./... && go vet ./... && golangci-lint run ./...` and summarize the commit series.
