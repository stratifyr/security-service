...
2. **Plan — Research and verify every claim:** Explore the referenced files; confirm the issue's statements are accurate (dead code really is unreferenced, structs really are triplicated, etc.). Use `make verify` at the start as a clean baseline.
...
7. **Build — Verify:** Run `make verify`.
...
12. **PR — Final verification:** Run `make verify` and summarize the commit series.
