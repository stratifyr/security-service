...
type securityServiceClient struct {
	grpcConn grpc.SecurityServiceGoFrClient
	cache    *redis.Client
}
...
func NewSecurityServiceClient(config config.Config, metricsManager metrics.Manager) (SecurityServiceClient, error) {
	...
	grpcConn, err := grpc.NewSecurityServiceGoFrClient(securityServiceHost, metricsManager)
	if err != nil {
...
func (c *securityServiceClient) GetMarketDays(ctx *gofr.Context, startDate, endDate time.Time) ([]time.Time, error) {
	resp, err := c.grpcConn.GetMarketDays(ctx, &proto.GetMarketDaysRequest{
...
func (c *securityServiceClient) GetMetrics(ctx *gofr.Context) ([]*proto.Metric, error) {
	...
	resp, err := c.grpcConn.GetMetrics(ctx, &proto.GetMetricsRequest{})
	if err != nil {
